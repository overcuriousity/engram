...
**Interfaces:**
- Consumes: nothing.
- Produces: `pub enum Error` with variants `NotFound`, `Duplicate { existing_id: String }`, `Validation(String)`, `Unauthorized`, `Forbidden`, `Inference { role: &'static str, source: String }`, `Vector(String)`, `Store(String)`, `MalformedLlmOutput(String)`; `Error::retryable(&self) -> bool`; `Error::status(&self) -> StatusCode`; `impl IntoResponse for Error`; `pub type Result<T> = std::result::Result<T, Error>`.
...
mod tests {
    ...
    fn retryable_only_for_transient_failures() {
        assert!(Error::Inference { role: "embed", source: "timeout".into() }.retryable());
    ...
    fn maps_to_expected_status_codes() {
        ...
        assert_eq!(
            Error::Inference { role: "chunk", source: "x".into() }.status(),
...
pub enum Error {
    ...
    #[error("forbidden")]
    Forbidden,
    #[error("inference[{role}]: {source}")]
    Inference { role: &'static str, source: String },
...
impl Chunker for FakeChunker {
    async fn segment(&self, text: &str) -> Result<Vec<ProposedChunk>> {
        if let Some(m) = &self.fail_with {
            return Err(Error::Inference { role: "chunk", source: m.clone() });
        }
        Ok(text
            .split("\n\n")
            .map(str::trim)
...
async fn post_json(
    ...
    let res = req.send().await.map_err(|e| Error::Inference { role, source: e.to_string() })?;
    ...
    if !status.is_success() {
        ...
        return Err(Error::Inference { role, source: format!("HTTP {status}: {detail}") });
    }
    res.json().await.map_err(|e| Error::Inference { role, source: e.to_string() })
}
...
impl HttpChunker {
    ...
    async fn chat(&self, messages: serde_json::Value) -> Result<String> {
        ...
        v["choices"][0]["message"]["content"]
            .as_str()
            .map(str::to_string)
            .ok_or_else(|| Error::Inference { role: "chunk", source: "no message content".into() })
    }
}
...
impl Embedder for HttpEmbedder {
    async fn embed(&self, texts: &[String]) -> Result<Vec<Vec<f32>>> {
        ...
        let data = v["data"].as_array().ok_or_else(|| Error::Inference {
            role: "embed", source: "response had no data array".into() })?;

        let mut out = vec![Vec::new(); texts.len()];
        for item in data {
            let idx = item["index"].as_u64().unwrap_or(0) as usize;
            let vec_f: Vec<f32> = item["embedding"].as_array()
                .ok_or_else(|| Error::Inference { role: "embed", source: "missing embedding".into() })?
            ...
            if idx >= out.len() {
                return Err(Error::Inference { role: "embed", source: format!("index {idx} out of range") });
            }
            out[idx] = vec_f;
        }
        if out.iter().any(Vec::is_empty) {
            return Err(Error::Inference { role: "embed", source: "endpoint skipped an input".into() });
        }
        Ok(out)
    }
...
impl Reranker for HttpReranker {
    async fn rerank(&self, query: &str, docs: &[String], top_n: usize) -> Result<Vec<(usize, f32)>> {
        ...
        let arr = v.as_array().cloned()
            ...
            .ok_or_else(|| Error::Inference { role: "rerank", source: "unrecognised response shape".into() })?;
...
impl Completer for HttpCompleter {
    async fn complete(&self, system: &str, user: &str) -> Result<String> {
        ...
        v["choices"][0]["message"]["content"]
            ...
            .ok_or_else(|| Error::Inference { role: "ask", source: "no message content".into() })
    }
