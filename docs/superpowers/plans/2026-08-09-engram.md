...
**Source spec:** `docs/superpowers/specs/2026-08-09-engram-design.md`

## Global Constraints

- Rust edition 2024. **MSRV 1.94** — our own code needs 1.88 for let-chains in the `Identity` extractor, but sqlx 0.9 raises the floor to 1.94. Do not lower it.
...
- Secrets (`ENGRAM__AUTH__OIDC__CLIENT_SECRET`, inference API keys) come from environment variables only. The config loader warns if it finds one in the file.
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(bind = %cfg.server.bind, "engram listening");
...
mod tests {
    ...
    async fn a_wrong_token_is_unauthorized() {
        ...
        for bad in ["engram_wrongwrongwrongwrongwrongwrongwrongwrong", "garbage", ""] {
...
- Produces:
  ...
  - `impl FromRequestParts<AppState> for Identity` — accepts `Authorization: Bearer engram_...` or the session cookie, rejects everything else with 401
...
mod tests {
    ...
    async fn a_bad_token_is_rejected() {
        let (app, _) = app_and_token().await;
        let res = app.oneshot(get("/api/v1/search?q=x", Some("engram_not_a_real_token"))).await.unwrap();
...
Run: `cargo test --bin engram startup`
...
Expected: log lines for the created Qdrant collection, the token-counting mode, and `engram listening`. `curl -s localhost:8080/healthz` returns `ok`; `curl -s localhost:8080/api/v1/status` returns 401.
...
<head>
  ...
  <title>{% block title %}engram{% endblock %}</title>
...
```html
{% extends "layout.html" %}
{% block title %}Sign in — engram{% endblock %}
{% block nav %}{% endblock %}
{% block content %}
<div class="card" style="max-width:22rem;margin:6rem auto">
  <div class="card-head"><span class="card-title">Sign in to engram</span></div>
  {% if oidc %}
...
```html
{% extends "layout.html" %}
{% block title %}Browse — engram{% endblock %}
{% block content %}
<table class="grid">
...
```rust
    ...
    async fn minting_a_token_shows_the_plaintext_exactly_once() {
        ...
        let html = body_of(res).await;
        assert!(html.contains("engram_"), "the token must be shown once: {html}");
        ...
        assert!(!page.contains("engram_"), "a stored token leaked into the ops page");
    }
...
#[derive(Clone)]
pub struct EngramTools { pub core: Core }
...
1. Annotate `impl EngramTools` with the tool macros so `ingest_tool`, `search_tool` and `ask_tool` are exported as `ingest`, `search` and `ask`, each with a description string taken from the doc comments above.
...
pub fn mcp_router() -> Router<AppState> {
    Router::new()
        .route_service("/mcp", /* rmcp streamable-http service built from EngramTools */)
...
use engram::vector::{SearchFilter, VectorPayload, VectorPoint, VectorStore, qdrant::QdrantVectors};
...
async fn upsert_search_and_payload_roundtrip() {
    let v = fresh("engram_it_roundtrip").await;
    v.upsert(vec![
...
async fn filtered_search_uses_payload_indexes() {
    let v = fresh("engram_it_filter").await;
    v.upsert(vec![
...
async fn upsert_is_idempotent_per_chunk_id() {
    let v = fresh("engram_it_idempotent").await;
...
async fn delete_by_source_removes_only_that_source() {
    let v = fresh("engram_it_delete").await;
    v.upsert(vec![
...
Create `src/lib.rs` declaring the public modules (`pub mod auth; pub mod config; pub mod core; pub mod error; pub mod infer; pub mod jobs; pub mod store; pub mod vector; pub mod web; pub mod mcp;`) and reduce `src/main.rs` to `use engram::...` plus the `main` function and its startup helpers.
...
Deliberately not built, per the design's post-v1 list: hybrid FTS + vector search (index and `keyword_search` exist, unused by the retrieval path), rerank enabled by default, the CLI, `engram eval`, OAuth 2.1 for `/mcp`, and file upload.
